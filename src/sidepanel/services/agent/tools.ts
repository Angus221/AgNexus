/**
 * AG.NEXUS - LangChain 工具定义 (TypeScript)
 */

import { DynamicStructuredTool } from '@langchain/core/tools'
import { z } from 'zod'
import { Storage } from '../storage'
import type { BookmarkInput, TodoInput, PromptInput, CommandInput, UserProfile } from '../types'
 ...
 * 获取当前标签页信息
 */
async function getCurrentTabInfo(): Promise<TabInfo | null> {
 ...
 * 从 URL 提取最后一级路径
 */
function extractLastPath(url: string): string {
  try {
    const urlObj = new URL(url)
...
export const addNavTool = new DynamicStructuredTool({
  ...
  func: async ({ title, url }: { title: string; url: string }) => {
    try {
      // 处理 {{current_tab}}
      ...
      await Storage.addNav({ title, url })
      return `✅ 已添加导航：${title}`
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      return `❌ 添加导航失败：${message}`
    }
  },
})
...
export const addBookmarkTool = new DynamicStructuredTool({
  ...
  func: async ({ title, url, description }: { title: string; url: string; description?: string }) => {
    try {
      // 处理 {{current_tab}}
      if (url === '{{current_tab}}' || title === '{{current_tab}}') {
        const tabInfo = await getCurrentTabInfo()
      ...
      await Storage.addBookmark(bookmarkInput)
      return `✅ 已收藏：${title}`
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      return `❌ 收藏失败：${message}`
...
export const addPromptTool = new DynamicStructuredTool({
  ...
  func: async (data: PromptInput) => {
    try {
      await Storage.addPrompt(data)
      return `✅ 已添加提示词：${data.title}`
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      return `❌ 添加提示词失败：${message}`
    }
  },
})
...
export const addCmdTool = new DynamicStructuredTool({
  ...
  func: async (data: CommandInput) => {
    try {
      await Storage.addCommand(data)
      return `✅ 已添加指令：${data.title}`
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      return `❌ 添加指令失败：${message}`
    }
  },
})
...
export const saveUserProfileTool = new DynamicStructuredTool({
  ...
  func: async (data: Partial<UserProfile>) => {
    try {
      const currentProfile = (await Storage.getUserProfile()) || { name: '', gender: '' }
